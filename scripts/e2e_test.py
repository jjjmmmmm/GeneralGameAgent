...
from common import BUTTONS, FPS, BTN_THRESHOLD, PRED_ROW, get_row, BUTTON_TO_MODEL_COL
...
for fi, sec, fid, pred, dt in preds:
    gt = get_row(fid)
    # 模型按钮：按 G1 键序映射（BUTTON_ACTION_TOKENS）取对应列
    pred_btn = np.array([
        int((pred["buttons"][PRED_ROW, BUTTON_TO_MODEL_COL[b]] > BTN_THRESHOLD))
        for b in BUTTONS
    ], dtype=int)
...
with OUT_MD.open("w", encoding="utf-8") as f:
    ...
    f.write(f"- 模型侧：取 `pred['buttons'][{PRED_ROW}, col]`，col 按 G1 键序映射表（官方 BUTTON_ACTION_TOKENS，见 docs/键序映射表.md）\n")
    ...
    f.write("- G1 键序映射采用官方 BUTTON_ACTION_TOKENS（源码法）；相关性法因模型预测稀疏无法二次确认（记录局限）\n")
    f.write("- G2 本验证仅 5 帧，非 M3 的 ≥200 帧测试集评测\n")
    f.write("- 模型预测稀疏：5 帧中多数无按键预测，按键准确率含大量 0==0，需以触发召回率为准（G3）\n")
