...
NitroGen 是 NVIDIA 开源的通用游戏智能体基础模型（500M 参数 DiT），输入单帧游戏画面，输出手柄动作块（实测 18 步，按键 + 双摇杆），经由互联网游戏视频的行为克隆训练得到。本项目的目标是：**在 9 天课程周期内，跑通该模型的离线推理链路，建立可复现、可核对的评测体系，并开发手柄动作可视化工具**，验证 zero-shot 条件下模型与人类操作标注的一致性。
...
# 即动作块（实测 18 步）：双摇杆连续值 + 21 键动作空间（键序见 docs/键序映射表.md）
```
